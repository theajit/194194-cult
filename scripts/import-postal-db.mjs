...
const csvPath=process.argv[2]||process.env.POSTAL_CSV_PATH;
if(!csvPath)throw new Error('Pass the official CSV path: npm run db:import:postal -- /path/to/pincode.csv');
...
try{
  ...
  let headers=null,indexes=null,batch=[],acceptedRows=0;
  ...
  try{
    ...
    console.log(`Imported ${counts.offices} post offices across ${counts.pins} unique PIN codes from ${acceptedRows} accepted CSV rows.`);
  }catch(error){
