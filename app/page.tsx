...
export default function Home(){
  ...
  useEffect(()=>{
    fetch('/api/postal/status',{cache:'no-store'}).then(async r=>({ok:r.ok,data:await r.json()})).then(({data})=>setPostalStatus(data)).catch(()=>setPostalStatus({configured:false,imported:false}));
...
{mode==='place'&&<div className="placeFinder"><input aria-label="Place or post office" value={place} onChange={e=>setPlace(e.target.value)} placeholder="Area, district or post office"/>{place.trim().length>=2&&<div className="searchResults">{placeResults.length?placeResults.map(r=><button key={`${r.pincode}-${r.office}`} onClick={()=>router.push(`/pincode/${r.pincode}`)}><strong>{r.pincode}</strong><span>{r.office}</span><small>{r.district} · {r.state} · {r.postOffices} office{r.postOffices===1?'':'s'}</small></button>):<p>No postal match found.</p>}</div>}</div>}
