...
function officeTypeLabel(officeName:string,officeType:string|null){
  const upper=officeName.toUpperCase();
  if(upper.includes(' B.O'))return 'Branch Post Office (B.O)';
  if(upper.includes(' S.O'))return 'Sub Post Office (S.O)';
  if(upper.includes(' H.O'))return 'Head Post Office (H.O)';
  return officeType&&officeType.toUpperCase()!=='PO'?officeType:'Post Office';
}

function deliveryInfo(status:string|null){
  const normalized=(status||'').toLowerCase().replace(/[-_]/g,' ').trim();
  const isNonDelivery=normalized.includes('non')&&normalized.includes('delivery');
  const isDelivery=!isNonDelivery&&normalized.includes('delivery');
  if(isDelivery)return {label:'Delivery Office',description:'Delivers mail to addresses in its assigned delivery area.'};
  if(isNonDelivery)return {label:'Non-Delivery Office',description:'Provides postal services, but doorstep delivery is handled by another delivery office.'};
  return {label:'Delivery status not specified',description:'The source postal directory does not specify a delivery role for this office.'};
}
...
export default async function PinPage({params}:{params:{pin:string}}){
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd)}}/><nav><Link href="/" className="brand"><b>194.194</b><span>CULT</span></Link><Link href="/" className="back">← SEARCH ANOTHER PIN</Link></nav><div className="breadcrumbs"><Link href="/pincodes">India</Link> / <Link href={`/pincodes/${stateSlug}`}>{d.state}</Link> / <Link href={`/pincodes/${stateSlug}/${districtSlug}`}>{d.district}</Link> / {d.pincode}</div><section className="pinHero"><div className={`status ${status.toLowerCase()}`}><span>●</span> {label[status]}</div><div className="pinNumber">{d.pincode}</div><h1>{d.district}<br/><i>{d.state}</i></h1><p>{d.postOffices.length} post office{d.postOffices.length===1?'':'s'} listed for PIN {d.pincode}.</p></section><section className="pinLayers"><article><span>POSTAL IDENTITY</span><h2>PIN CODE</h2><div className="officeGrid">{d.postOffices.map(o=>{const delivery=deliveryInfo(o.deliveryStatus);return <article key={o.officeName}><h3>{o.officeName}</h3><p><b>{officeTypeLabel(o.officeName,o.officeType)}</b></p><p><b>{delivery.label}</b><br/>{delivery.description}</p><small>{d.district}, {d.state} — {d.pincode}</small>{o.divisionName&&<small><br/>Division: {o.divisionName}</small>}{o.regionName&&<small><br/>Region: {o.regionName}</small>}</article>})}</div></article><article><span>PRECISE LOCATION</span><h2>DIGIPIN</h2><p>A PIN identifies a postal area. DIGIPIN identifies a precise location within that area, so one PIN can contain many DIGIPINs.</p><a className="textCta" href={DIGIPIN_SITE} target="_blank" rel="noreferrer">OPEN DIGIPIN →</a></article></section>{status==='ACTIVE'?<section className="chapter"><span>194.194 CULT · CHAPTER</span><h2>{cult.chapterName||'Active Chapter'}</h2>{cult.hostLocation==='Pin Code Café'&&<a className="foundingCafe" href="https://pincode.cafe" target="_blank" rel="noreferrer"><img src="https://pincode.cafe/pin-code-cafe-logo.jpg" alt="Pin Code Café"/><div><small>BIRTHPLACE OF 194.194 CULT</small><b>Pin Code Café · Dhenkanal</b><em>pincode.cafe →</em></div></a>}<dl><div><dt>HOME</dt><dd>{cult.hostLocation||d.district}</dd></div><div><dt>SINCE</dt><dd>{cult.sinceYear||'—'}</dd></div><div><dt>STATUS</dt><dd>ALIVE & BREWING ☕</dd></div></dl><blockquote>“This PIN has a Cult.”</blockquote></section>:status==='FORMING'?<section className="cta"><span>194.194 CULT · {d.pincode}</span><h2>This PIN is<br/>forming a Cult.</h2><p>The community is taking shape here.</p></section>:<section className="cta"><span>194.194 CULT · {d.pincode}</span><h2>Not here yet.<br/>Be the reason it starts.</h2><a className="textCta" href={cultMailto}>BRING 194.194 CULT HERE →</a><p>Email us your interest. When enough people raise their hands, this PIN starts forming.</p></section>}<footer><b>194.194 CULT</b><span>PIN CODE · DIGIPIN · COMMUNITY</span><span>PIN DATA: INDIA POST / OGD</span></footer></main>;
}
