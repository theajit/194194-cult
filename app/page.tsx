...
type PinRecord={pincode:string;district:string;state:string;postOffices:{officeName:string}[]};
...
export default function Home(){
  ...
  const placeResults=useMemo(()=>{const q=place.trim().toLowerCase();if(q.length<2)return [];return records.filter(r=>r.district.toLowerCase().includes(q)||r.state.toLowerCase().includes(q)||r.postOffices.some(o=>o.officeName.toLowerCase().includes(q))).slice(0,12)},[place,records]);
...
{mode==='place'&&<div className="placeFinder"><input aria-label="Place or post office" value={place} onChange={e=>setPlace(e.target.value)} placeholder="Area, district or post office"/>{place.trim().length>=2&&<div className="searchResults">{placeResults.length?placeResults.map(r=><button key={r.pincode} onClick={()=>router.push(`/pincode/${r.pincode}`)}><strong>{r.pincode}</strong><span>{r.postOffices[0]?.officeName||r.district}</span><small>{r.district} · {r.state}</small></button>):<p>No verified local match yet. Odisha directory rollout is in progress.</p>}</div>}</div>}
{mode==='browse'&&<div className="browseStates">{states.map(state=><button key={state} onClick={()=>router.push(`/pincodes/${state.toLowerCase().replace(/&/g,'and').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}`)}><span>STATE / UT</span><b>{state}</b><small>Browse districts and PIN codes →</small></button>)}<button className="allIndia" onClick={()=>router.push('/pincodes')}><span>DIRECTORY</span><b>ALL STATES</b><small>Open India PIN directory →</small></button></div>}
