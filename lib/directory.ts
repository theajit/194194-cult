...
export const states=()=>Array.from(new Set(records.map(r=>r.state))).sort();
export const districtsForState=(stateSlug:string)=>Array.from(new Set(records.filter(r=>slug(r.state)===stateSlug).map(r=>r.district))).sort();
